...
public class InventoryOptionsHandler {
	...
	public static void handleItemOption1(final Player player, final int slotId, final int itemId, Item item) {
		...
		if (itemId == 2724) {
				...
				player.getPackets().sendGameMessage("You clicked a box");
	...
	public static void handleInterfaceOnInterface(final Player player, InputStream stream) {
		System.out.println("TEST CLICK 10");
		...
		if ((interfaceId == Inventory.INVENTORY_INTERFACE || interfaceId == Inventory.INVENTORY_INTERFACE_2)
			...
			if (!player.getControllerManager().canUseItemOnItem(itemUsed, usedWith))
				...
				if (player.getInventory().containsItem(30372, 1) && player.getInventory().containsItem(usedWithId, 1)) {
					...
					|| (itemUsedId == ItemIdentifiers.RING_OF_DEATH || itemUsedId == ItemIdentifiers.REAPER_NECKLACE || itemUsedId == ItemIdentifiers.AMULET_OF_SOULS || itemUsedId == ItemIdentifiers.DEATHTOUCH_BRACELET)
					|| (usedWithId == ItemIdentifiers.RING_OF_DEATH || usedWithId == ItemIdentifiers.REAPER_NECKLACE || usedWithId == ItemIdentifiers.AMULET_OF_SOULS || usedWithId == ItemIdentifiers.DEATHTOUCH_BRACELET))) {
				int uncharged = itemUsedId == ItemIdentifiers.ONYX ? usedWithId : itemUsedId;
				int currentcharge = player.getCharges().getCharges(uncharged);
				//player.getPackets().sendGameMessage("Charge Amount: "+String.valueOf(currentcharge));
				if(currentcharge == 100000){
					player.getPackets().sendGameMessage("Your hydrix jewelry is already fully charged.");
					skipnih = true;
				}else {
			...
			if (herb != null) {
				...
				return;
			else if ((itemUsedId == 453 && usedWithId == 18339) || (itemUsedId == 18339 && usedWithId== 453)){
				Coalbag.addCoal(player);
			} else if ((itemUsedId == 18339 && usedWithId == 454) || (itemUsedId == 454 && usedWithId == 18339)) {
				...
				if(skipnih==true){skipnih=false;}else{player.getPackets().sendGameMessage("Nothing interesting happens.");}
	...
	public static void handleItemOption3(Player player, int slotId, int itemId, Item item) {
		System.out.println("TEST CLICK 3");
		if (player.isLocked() || player.isStunned() || player.getEmotesManager().isDoingEmote())
			return;
		player.stopAll(false);
		...
		}else if(itemId==19675) {
			System.out.println("Adding Herbicide to toolbelt");
	...
	public static void handleItemOption4(Player player, int slotId, int itemId, Item item) {
		System.out.println("TEST CLICK 4");
	...
	public static void handleItemOption5(Player player, int slotId, int itemId, Item item) {
		System.out.println("TEST CLICK 5");
	...
	public static void handleItemOption6(Player player, int slot, int itemId, Item item) {
		System.out.println("TEST CLICK 6");
		if (player.isLocked() || player.isStunned() || player.getEmotesManager().isDoingEmote())
			return;
		player.stopAll(false);
		switch (itemId) {
		...
		if (grind != null) {
			...
			player.getPackets().sendGameMessage("Nothing interesting happens. Dickhead");
	}

	public static void handleItemOption7(Player player, int slotId, int itemId, Item item) {
		System.out.println("TEST CLICK 7");
	...
	public static void handleItemOption8(Player player, int slotId, int itemId, Item item) {
		System.out.println("TEST CLICK 8");
		if (player.isLocked() || player.isStunned() || player.getEmotesManager().isDoingEmote())
			return;
		if (!player.getControllerManager().canDropItem(item))
			return;
