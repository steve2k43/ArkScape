...
public final class ObjectHandler {
	...
	private static void handleOption1(final Player player,
		...
		player.setRouteEvent(new RouteEvent(
				...
				() -> {
						...
						case 15468:
							if (player.getInventory().containsItem(Smithing.HAMMER, 1)) {
								player.getPackets().sendGameMessage(
										"You already have a hammer in your inventory.");handled = true;
								return;
							}
							if (player.getInventory().getFreeSlots() < 1) {
								player.getPackets().sendGameMessage(
										"You do not have enough inventory space.");handled = true;
								return;
							}
							player.getDialogueManager().startDialogue("SimpleItemMessage",
									Smithing.HAMMER, "You take a hammer from the crate.");
							player.getInventory().addItem(Smithing.HAMMER, 1);handled = true;
							break;


					}
					if (object.getId() == 106004) {
						System.out.println("Xmas Cupboard");
						player.getMusicsManager().playMusic(1);
						//handled = true;
					}
					else if (object.getId() == 79519) {
						RunespanController.enterRunespan(player, false);handled = true;
					}
					else if (object.getId() == 79520) {
						RunespanController.enterRunespan(player, true);handled = true;
					}
					else if (id == 79770) {
						...
						new WorldTile(2771, 10161, 0);handled = true;

					}
					else if (id == 5014) {
						new WorldTile(2728, 3707, 0);handled = true;

					}
					else if (id == 5973) {
						new WorldTile(2836, 10144, 0);handled = true;

					}
					else if (id == 57050) {
						new WorldTile(1519, 4704, 0);handled = true;

					}
					else if (id == 13967 || id == 13976 || id == 13974) {
						Magic.sendDemonicTeleport(player, new WorldTile(3297, 9824, 0));
						player.getPackets().sendGameMessage(
								"You are dragged beyond the depths of this mortal world");handled = true;
					}
					else if (id == 13999) {
						Magic.sendSkyJumpTeleport(player, new WorldTile(
								Settings.HOME_LOCATION));handled = true;
					}
					else if (id == 45008 || id == 45009) {
						new WorldTile(2821, 10212, 0);handled = true;
					...
					else if (id == 6087 && player.getX() == 2893) {
					...
					else if (id == 104065) { // stone of apollo
						objectDef.name = "Stone Of Apollo";
						player.getDialogueManager().startDialogue("ApolloDialogue");
						player.getPackets().sendGameMessage(
								"You have consulted with the Great Stone Of Apollo");handled = true;
						return;
					}
					else if (id == 94077) {
						player.setRouteEvent(new RouteEvent(object, () -> {
					 ...
					 */
						...
						if (n.getX() == 3097 && n.getY() == 6119) {
							...
							player.getPackets().sendEntityMessage(1, 15263739, player,
									"Vorago isn't in the right place!");handled = true;
						}
						handled=true;
					}

					else if (object.getId() == 84828 && player.getX() > object.getX()) {
						player.setNextWorldTile(new WorldTile(object.getX() - 1, object
								.getY(), 0));handled = true;
					}

					else if (object.getId() == 56990) {
						player.setNextWorldTile(new WorldTile(2177, 5664, 0));handled = true;
					}

					else if (object.getId() == 56989) {
						player.setNextWorldTile(new WorldTile(2939, 10197, 0));handled = true;
					}
					// Magic.sendNormalTeleportSpell(player, 1, 0, Vorago.OUTSIDE);
					...
					else if (object.getId() == 84826) {// Gaps TODO Anims
					...
					else if (Ectofuntus.handleObjects(player, object.getId())) {
						handled = true;
						return;
					}
						...
						player.setRouteEvent(new RouteEvent(object, () -> player
								.getControllerManager().processObjectClick1(object), true));handled = true;
						return;
					}
					else if (id == 87989) {
						player.setRouteEvent(new RouteEvent(object, () -> {
						...
						player.setNextWorldTile(new WorldTile(2910, 10176, 0));handled = true;
					}
					else if (id == 43529 || id == 69514 || (id >= 4550 && id <= 4559)) {
						player.setRouteEvent(new RouteEvent(object, () -> {
						...
						player.getControllerManager().processObjectClick1(object);handled = true;
						return;
					...
					if (object.getId() == 97272) {
						System.out.println("Accessed world window");
						WorldTasksManager.schedule(new WorldTask() {
							...
							public void run() {
								...
								player.useStairs(
										9016,
										new WorldTile(4316, 781, 1), 4, 5, null, true);
							}
						}, 2);
						handled = true;
					}
					if (object.getId() == 97808) { //mazcab portal
						System.out.println("Accessed world window");
						WorldTasksManager.schedule(new WorldTask() {
							...
							public void run() {
								...
								player.useStairs(
										9016,
										new WorldTile(3103, 2981, 3), 4, 5, null, true);
							}
						}, 2);
						handled = true;
					}
					if (object.getId() == 97599) {
						...
						WorldTasksManager.schedule(new WorldTask() {
							...
							public void run() {
								...
								player.useStairs(
										844,
										new WorldTile(3112, 3039, 3), 4, 5, null, true);
							}
						}, 1);
						handled = true;
					}
					if (object.getId() == 97600) { // Mazcab
						WorldTasksManager.schedule(new WorldTask() {
							...
							public void run() {
								...
								player.useStairs(
										844,
										new WorldTile(3138, 3006, 0), 4, 5, null, true);
							}
						}, 1);
						handled = true;
					}
						...
						if (player.getX() < 2306) {
							WorldTasksManager.schedule(new WorldTask() {
								...
								public void run() {
									...
									player.useStairs(
											844,
											new WorldTile(player.getX() + 3, player.getY() + 1, 0), 4, 5, null, true);
								}
							}, 1);
							handled = true;
						} else {
					...
					if (object.getId() == 87306) {
						...
						player.useStairs(-1, new WorldTile(1772, 5366, 0), 0, 1);handled = true;
					 ...
					 */
						...
						if (player.getSkills().getLevel(Skills.DIVINATION) < 75) {
							player.getPackets()
									.sendGameMessage(
											"You need to have a divination level of at least 75 to use this.");handled = true;
							return;
						}
						...
						switch (Utils.random(3)) {
							case 0:
								player.getPackets()
										.sendGameMessage(
												"You don't know what's waiting for you the in next corner.");handled = true;
								break;
							case 1:
								player.getPackets().sendGameMessage(
										"Never say never.");handled = true;
								break;
						}
					} else if (id == 94058) {
						player.getActionManager().setAction(new Harps(object));handled = true;
						return;
					} else if (id == 94048) {
						if (player.getSkills().getLevel(Skills.PRAYER) < 75) {
							player.getPackets()
									.sendGameMessage(
											"You need to have a prayer level of at least 75 to use this.");handled = true;
							return;
						}
						player.getActionManager().setAction(
					...
					// kalphite king
						...
						player.getPackets().sendGameMessage("You start to jump the rock");
						...
						if(id==2335) {
							WorldTasksManager.schedule(new WorldTask() {
								...
								public void run() {
									player.useStairs(
											3303,
											new WorldTile(player.getX(), player.getY() + 2, 0), 4, 5, null, true);
								}
							}, 1);handled = true;
						}
						if(id==2333) {
						...
						WorldTasksManager.schedule(new WorldTask() {
							...
							public void run() {
								player.useStairs(
										3303,
										new WorldTile(player.getX(),player.getY()+yy, 0), 4, 5, null, true);
							}
						}, 1);handled = true;
					}
					// entrana dungeon
						...
						player.setNextWorldTile(new WorldTile(2984, 3382, 0));handled = true;
					}else if (id == 91557) {
					...
					if (object.getId() == 16543) {
						...
						Runecrafting.craftZMIAltar(player);handled = true;
						return;
					}
					// heroes guild
						...
						World.spawnObjectTemporary(opened, 1000);
						player.addWalkSteps(player.getX() == x ? x - 1 : x,
								object.getY(), 1, false);handled = true;
					} else if (id == 67346 && x >= 2906 && (y >= 3512 && y <= 3516)) {
					...
					// start of death platue
						...
						player.useStairs(-1, new WorldTile(1178, 6355, 0), 1, 2);handled = true;
					} else if (id == 70796) {
						player.useStairs(-1, new WorldTile(1090, 6360, 0), 1, 2);handled = true;
					} else if (id == 70798) {
						player.useStairs(-1, new WorldTile(1340, 6380, 0), 1, 2);handled = true;
					} else if (id == 70797) {
						player.useStairs(-1, new WorldTile(1090, 6497, 0), 1, 2);handled = true;
					} else if (id == 70792) {
						player.useStairs(-1, new WorldTile(1206, 6371, 0), 1, 2);handled = true;
					} else if (id == 70793) {
						player.useStairs(-1, new WorldTile(2989, 3237, 0), 1, 2);handled = true;
					} else if (id == 2353
					...
					// lighthouse dungeon
						...
						player.useStairs(-1, new WorldTile(3374, 3093, 0), 0, 1);handled = true;
					} else if (id == 62674) { // dominion entrance
						player.useStairs(-1, new WorldTile(3744, 6405, 0), 0, 1);handled = true;
						...
						switch (objectDef.name.toLowerCase()) {
							...
							case "anvil":
								if (objectDef.containsOption(0, "Smith")) {
									...
									System.out.println(ForgingBar.getBar(player));
								}
								handled = true;
								break;
							case "portable forge":
					...
					callit(handled, id, object, x, y, player.getPlane());
					handled=false;
	...
	private static void callit(boolean handled, int id, WorldObject object, int x, int y, int z) {
		final ObjectDefinitions objectDef = object.getDefinitions();
		System.out.println("Clicked Object: "+ id+" , Handled = "+handled);
		if(!handled) {
			Bugsystem.addObjectBug(1, objectDef.name, id,x,y,z);
