...
import net.nocturne.tools.ItemIdToName;
...
public final class WorldPacketsDecoder extends Decoder {
	...
	private final static int TELEGRAB_PACKET = 64;
	...
	public void processPackets(final int opcode, InputStream stream)
		...
		if (opcode == PING_PACKET) {
			...
			} else if (player.getTemporaryAttributtes().remove("bug") != null) {
				//System.out.println(value);
				//TicketSystem.addTicket(new TicketEntry(player, value));
				//Bugsystem.addBug(new BugEntry(player, value));
				Bugsystem.addBug(player, value, player.getUsername());
				player.getDialogueManager().startDialogue("SimpleMessage",
			...
			if (Settings.DEBUG)
				System.out.println("Switch item FIID: " + fromInterfaceId + ", TIID: "
						+ toInterfaceId + ", FS: " + fromSlot + ", TS: " + toSlot
						+ ", FCID: " + fromComponentId + ", TCID: " + toComponentId);
			...
			//player.getPackets().sendGameMessage(ItemExamines.getExamine(itemId);
		...
		} else if (opcode == TELEGRAB_PACKET) {
			int[][] unsigned = new int[5][10];
			int[][] signed = new int[4][3];
			...
			if (!player.getMapRegionsIds().contains(regionId))
				return;
			final FloorItem item = World.getRegion(regionId).getGroundItem(itemId,
					tile, player);
			if (item == null)
				return;
			...
			if (player.getEquipment().getWeaponId() == 1381 || player.getEquipment().getWeaponId() == 1397
				...
				if (!player.getInventory().containsItem(563, 1)
						|| !player.getInventory().containsItem(556, 1)) {
					player.getPackets()
							.sendGameMessage(
									"You do not have the required runes to cast this spell.");
					return;
			...
			int x = b + (c * 256);
			int y = stream.readUnsignedShort(); // y
			int id = stream.readUnsignedShort(); // id
			int newid = id ^ 128;
			int newy = y ^ 128;
			boolean forceRun = stream.read128Byte() == 1;
			String Examine = ItemExamines.getExamine(new Item(newid));
			WorldTile tile = new WorldTile(x, newy, player.getPlane());
			FloorItem floorItem = World.getRegion(tile.getRegionId()).getGroundItem(newid, tile, player);

			player.getPackets().sendFloorItemMessage(0, 15263739, floorItem, Examine);
