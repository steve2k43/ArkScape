...
public class NPC_BACKUP extends Entity implements Serializable {
	...
	public NPC_BACKUP(int id, WorldTile tile, int mapAreaNameHash,
	 ...
	 * creates and adds npc
	 */
	public NPC_BACKUP(int id, WorldTile tile) {
		super(tile);
		this.id = id;
		this.respawnTile = new WorldTile(tile);
		combatLevel = -1;
				...
				if (loop == deathDelay) {
					if (source instanceof Player) {
						((Player) source).getControllerManager()
								.processNPCDeath(NPC_BACKUP.this);
					}
					giveXP();
					drop();
					reset();
					...
					if (!isSpawned())
						setRespawnTask();
					if (source.getAttackedBy() == NPC_BACKUP.this) { // no need to wait
						// after u kill
	...
	private void lootBeam(Player player, NPC_BACKUP npc, ItemDefinitions drop) {
			...
			if (checkNPCs) {
				...
				if (npcsIndexes != null) {
					for (int npcIndex : npcsIndexes) {
						NPC_BACKUP npc = World.getNPCs().get(npcIndex);
						if (npc == null
	...
	public boolean addWalkStepsInteract(final int destX, final int destY,
		...
		while (true) {
			...
			if (myY < destY)
				...
				myY--;
			if ((this instanceof NPC_BACKUP && !canWalkNPC(myX, myY))
