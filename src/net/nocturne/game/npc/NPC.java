...
public class NPC extends Entity implements Serializable {
	...
	public void drop() {
		...
		Drops drops = npcdropsSQL.getmydrop(id);
		if (drops == null)
			return;
		List<Player> players = FriendsChat.getLootSharingPeople(killer);
		int c = NPCDefinitions.getNPCDefinitions(id).combatLevel;
		String name = NPCDefinitions.getNPCDefinitions(id).getName();

		System.out.println(killer.getName()+" killed a "+name+" and received "+drops );
		List<Drop> dropp = drops.getAllDrops();
		List<Drop> itemswon = new ArrayList<>();
		System.out.println("DROP COUNT: "+dropp.size());
		System.out.println("PLAYER DC: "+killer.getDropChance());
		String itemname = "";
		int id= 0;
		int min;
		int max;
		...
		boolean alreadywonrare = false;
		boolean alreadywonveryrare = false;
		boolean alreadywonraretable = false;

		for (int i = 0; i < drops.getAllDrops().size(); i++) {

			id = dropp.get(i).getItemId();
			min = dropp.get(i).getMinAmount();
			max = dropp.get(i).getMaxAmount();
			rarity = dropp.get(i).getRarity();
			dcstring=dropp.get(i).getDropchance();
			try {
				String explode[]=dcstring.split("/");
				double a = Double.parseDouble(explode[0]);
				double b = Double.parseDouble(explode[1]);
				dropchance = Utils.round(a/b, 4);
				dropchance = Utils.round(dropchance * 100, 2);
			} catch (Throwable e) {
				Logger.handle(e);
			}
			if(dropchance==0.0) {
				int difficulty = killer.getDifficulty();
				if(difficulty==1)
					Math.ceil(dropchance = 100/Drops.DROP_RATES[rarity] / killer.getDropChance() * 100);
				if(difficulty==2)
					Math.ceil(dropchance = 100/Drops.DROP_RATES_NORMAL[rarity] / killer.getDropChance() * 100);
				if(difficulty==3)
					Math.ceil(dropchance = 100/Drops.DROP_RATES_EXTREME[rarity] / killer.getDropChance() * 100);
			}
			itemname = ItemDefinitions.getItemDefinitions(id).getName();
			//System.out.println("ITEM #"+i+" - ID: "+id+" - NAME: "+itemname+" Dropchance: 1/"+dropchance+" ("+100/dropchance*100+"%) R("+rarity+")");
			double roll;
			double chance = Utils.round((( 100/dropchance)*100), 2);
			roll = Utils.round(Utils.random(1, chance), 0);
			System.out.println("Rolling for: "+itemname);
			System.out.println("Roll: "+roll);
			System.out.println("Drop Chance: "+dropchance+"%");
			System.out.println("chance: 1/"+chance/100+"");
			if(roll <= 100) {
				if(rarity==4 && alreadywonveryrare==true){
					System.out.println("ALREADY WON VERY RARE ("+itemname+")");
				}
				else if(rarity==4 && alreadywonveryrare==false){
					alreadywonveryrare=true;
					System.out.println("LOOT: " + ItemDefinitions.getItemDefinitions(id).getName());
					//sendDrop(killer, new Drop(id, min, max));
					itemswon.add(new Drop(id, min, max));
				}
				if(rarity==3 && alreadywonrare==true){
					System.out.println("ALREADY WON RARE ("+itemname+")");
				}
				else if(rarity==3 && alreadywonrare==false){
					alreadywonrare=true;
					System.out.println("LOOT: " + ItemDefinitions.getItemDefinitions(id).getName());
					//sendDrop(killer, new Drop(id, min, max));
					itemswon.add(new Drop(id, min, max));
		...
		if (players == null || players.size() == 1) {
			...
			for (Drop drop : itemswon) {
				...
				if (hasHerbicide
						|| killer.getGamePointManager().hasGamePointsReward(
						GPR.SKILL_DROPS)) {
					System.out.println("PLAYER HAS HERBICIDE!");
				...
				System.out.println("DROP: "+drop.getItemId());
			}
		} else {
