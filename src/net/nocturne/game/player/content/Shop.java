...
public class Shop {
	...
	public void sellAll(Player player, int slotId) {
		if (slotId >= getStoreSize())
			return;
	...
	public void sendInfo(Player player, int slotId, boolean inventory) {
		...
		int test = item.getDefinitions().getPrice();
		player.getPackets().sendGameMessage(
	...
	private int getSellPrice(Item item) {
		...
		if (price > 0)
			return price / 10;
		return Math.max(1, (item.getDefinitions().getPrice() * 30) / 100);
	}
