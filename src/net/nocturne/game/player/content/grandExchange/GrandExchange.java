...
public class GrandExchange {
	...
	public static Offer getOffer(Player player, int slot) {
		player.getPackets().sendGameMessage("offer "+slot);
		synchronized (LOCK) {
	...
	public static void abortOffer(Player player, int slot) {
		synchronized (LOCK) {
			...
			System.out.println("LOL: "+player.getGeManager().getOfferUIds()[slot]);
			offer.cancel();
			System.out.println("offer remove");
	...
	public static void collectItems(Player player, int slot, int invSlot,
		...
		synchronized (LOCK) {
			...
			if (offer.collectItems(invSlot, option) && offer.forceRemove()) {
				...
				player.getInventory().addItem(new Item(offer.getId()));
			}
		}
	}
