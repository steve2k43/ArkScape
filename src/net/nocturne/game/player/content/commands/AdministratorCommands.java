...
class AdministratorCommands {
	...
	static boolean processCommand(final Player player, String[] cmd,
		...
		switch (cmd[0].toLowerCase()) {
			...
			case "savemeplz":
				try{
				File f = new File("C:/poo/"+player.getUsername()+".p");
		...
		case "inters":
			...
			try {
				...
				for (int componentId = 0; componentId < Utils
					...
					System.out.print(Utils.getInterfaceDefinitionsComponentsSize(interId));
				}
			} catch (NumberFormatException e) {
				player.getPackets().sendPanelBoxMessage(
						"Use: ::inter interfaceId");
		...
		case "testlucilledia":
			...
			return true;
			case "set":
				player.setinternumber(Integer.parseInt(cmd[1]));
				return true;
			case "h":
				int command = Integer.parseInt(cmd[1]);
				try {
					player.getPackets().sendHideIComponent(command, Integer.parseInt(cmd[2]), true);
				} catch (NumberFormatException e) {
					player.getPackets().sendPanelBoxMessage(
							"Use: ::inter interfaceId");
				}
			...
			case "blank":
				...
				for (int i = 0; i < Utils
						.getInterfaceDefinitionsComponentsSize(iface); i++)
					player.getPackets().sendIComponentText(iface, i, "");
