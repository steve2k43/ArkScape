...
public class GrandExchangeManagercopy implements Serializable {
	...
	public GrandExchangeManagercopy() {
	...
	public void handleButtons(int interfaceId, int componentId, int slotId, int packetId) {
		System.out.println("IID: "+interfaceId+" CID: "+componentId+" SID: "+slotId+" Packet: "+packetId);
		if (interfaceId == 105) {
			switch (componentId) {
			case 15: // converted
				cancelOffer();
	...
	public void viewOffer(int slot) {
		player.getPackets().sendGameMessage("viewing"+slot);
		if (isSlotFree(slot) || getCurrentSlot() != -1) {
			player.getPackets().sendGameMessage("code1");
			return;
		}
		Offer offer = GrandExchange.getOffer(player, slot);
		player.getPackets().sendGameMessage("code2");
		if (offer == null) {
			player.getPackets().sendGameMessage("code null");
			return;
		}
		setSlot(slot);
		player.getPackets().sendGameMessage("code3");
		setExtraDetails(offer.getId());
		player.getPackets().sendGameMessage("code4");
	}
