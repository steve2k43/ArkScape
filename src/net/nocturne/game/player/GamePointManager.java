...
public class GamePointManager implements Serializable {
		...
		if (gamePointType <= 1)
			player.getPackets().sendGameMessage(
					"<col=FF8330><shad=00001A>You have received "
							...
							+ " Arkpoints, you now have a total of "
							+ Utils.getFormattedNumber(gamePoints)
							+ " Arkpoints.");
		if (gamePointType == 0 && Utils.random(5) == 0)
			player.getPackets()
					.sendGameMessage(
							"<col=FF8330><shad=00001A>If you want the Arkpoint message shown, "
									+ "type ::showarkpoints. If you want them hidden, type ::hidearkpoints.");
		if (gamePointRewardType == 0 && gamePoints >= 5000
				&& Utils.random(5) == 0)
			player.getPackets()
					.sendGameMessage(
							"<col=FF8330><shad=00001A>Congratulations!"
									+ " You now have enough Arkpoints for a reward! Check out ::arkpointrewards for more information.");
	}
	...
	public void gamePointsReward(int index, String rewardMessage) {
		...
		if (gamePointRewardType <= 1)
			player.getPackets()
					.sendGameMessage(
							"<col=FF8330><shad=00001A>If you want to check your time left on your Arkpoint reward, type ::checkarkpoints.");
		gamePoints -= points;
	...
	public boolean hasGamePointsReward(GPR gpr, boolean sendMessage) {
		...
		if (rewardIsActive && sendMessage)
			if (rewardTimeIsUp) {
				player.getPackets().sendGameMessage(
						"<col=FF8330><shad=00001A> Your Arkpoints reward, "
