...
public final class Woodcutting extends Action {
	...
	private boolean checkAll(Player player) {
		int[][] toolbelt = player.getToolbelt().getToolbeltItems();
		hatchet = getHatchet(player, definitions.ordinal() > 22);
		System.out.println("Detected Hatchet 2: "+hatchet);
		if (hatchet == null) {
			player.getPackets()
					.sendGameMessage(
	...
	public static HatchetDefinitions getHatchet(Player player,
		...
		for (int i = dungeoneering ? 10
			...
			if (player.getInventory().containsItemToolBelt(def.itemId)
				...
				System.out.println("I = "+i+" AXE1: " + def);
				if (player.getSkills().getLevel(Skills.WOODCUTTING) >= def.levelRequired) {
					System.out.println("AXE2: " + def);
					return def;
				} else {
					System.out.println("Not required woodcutting level. Need: "+def.levelRequired);
				}
				System.out.println("AXE3: " + def);
			}
		}
		return null;
	}
